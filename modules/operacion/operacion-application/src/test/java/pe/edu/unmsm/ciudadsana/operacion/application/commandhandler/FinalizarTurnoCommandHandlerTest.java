...
class FinalizarTurnoCommandHandlerTest {
    ...
    void finalizar_turno_en_curso_retorna_success() {
        ...
        FinalizarTurnoCommand command = new FinalizarTurnoCommand(TURNO_ID, TENANT_ID);

        Result<Void> result = handler.finalizar(command);

        assertThat(result.isSuccess()).isTrue();
        verify(turno).finalizar(any());
        verify(turnosPersistencePort).save(turno);
        verify(eventPublisher).publishAll(any());
    }

    @Test
    void finalizar_turno_no_encontrado_retorna_failure() {
    ...
    @Test
    void finalizar_turno_no_en_curso_retorna_turno_invalido() {
        Turno turno = mock(Turno.class);
        when(turnosPersistencePort.findById(any(), any())).thenReturn(Optional.of(turno));
        doThrow(new IllegalStateException("Solo se puede finalizar un turno EN_CURSO. Estado actual: PROGRAMADO"))
                .when(turno).finalizar(any());
