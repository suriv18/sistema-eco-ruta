...
import pe.edu.unmsm.ciudadsana.rutas.application.port.in.ActualizarParadaUseCase;
...
@Component
public class ActualizarParadaCommandHandler implements ActualizarParadaUseCase {

    private final RutasPersistencePort rutasPersistencePort;
    private final RutasEventPublisherPort eventPublisher;

    public ActualizarParadaCommandHandler(RutasPersistencePort rutasPersistencePort,
    ...
    @Override
    public Result<RutaResponseDto> actualizar(ActualizarParadaCommand cmd) {
        ...
        RutaVersion versionActual = ruta.getVersionActual().orElse(null);
        if (versionActual == null) {
            return Result.failure(ErrorCode.RUTA_ESTADO_INVALIDO);
        }
        RutaParada parada = versionActual.getParadas().stream()
                .filter(p -> p.getId().value().equals(cmd.paradaId()))
                .findFirst()
                .orElse(null);
        if (parada == null) {
            return Result.failure(ErrorCode.RECURSO_NO_ENCONTRADO);
        }
        EstadoParada nuevoEstado = EstadoParada.valueOf(cmd.nuevoEstado());
        try {
            switch (nuevoEstado) {
                case EN_ATENCION -> parada.iniciarAtencion();
                case ATENDIDA -> parada.marcarAtendida(cmd.horaLlegada(), cmd.horaSalida());
                case OMITIDA -> parada.marcarOmitida();
                default -> throw new IllegalStateException("Estado no permitido: " + nuevoEstado);
            }
