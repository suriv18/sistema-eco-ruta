...
import pe.edu.unmsm.ciudadsana.rutas.application.dto.RutaEventoDto;
...
import pe.edu.unmsm.ciudadsana.rutas.application.port.in.ObtenerRutaDetalleUseCase;
...
@Component
public class ObtenerRutaDetalleQueryHandler implements ObtenerRutaDetalleUseCase {

    private final RutasPersistencePort rutasPersistencePort;

    public ObtenerRutaDetalleQueryHandler(RutasPersistencePort rutasPersistencePort) {
        this.rutasPersistencePort = rutasPersistencePort;
    }

    @Override
    public Result<RutaDetalleResponseDto> obtenerDetalle(ObtenerRutaDetalleQuery query) {
        ...
        return Result.success(toDetalleDto(ruta));
    }
    ...
    private RutaEventoDto toEventoDto(RutaEvento e) {
        return new RutaEventoDto(
                e.getId().value(), e.getRutaId().value(), e.getTipoEvento().name(),
                e.getDescripcion().orElse(null), e.getDatosJson().orElse(null),
                e.getCreadoEn()
        );
    }

    private RutaDetalleResponseDto toDetalleDto(Ruta r) {
        return new RutaDetalleResponseDto(
                ...
                r.getHistorialVersiones().stream().map(this::toVersionDto).toList(),
                r.getEventos().stream().map(this::toEventoDto).toList(),
