...
CREATE TABLE IF NOT EXISTS auth.usuario (
    ...
    creado_en       TIMESTAMPTZ NOT NULL DEFAULT now(),
    actualizado_en  TIMESTAMPTZ,
    CONSTRAINT fk_auth_usuario_tenant FOREIGN KEY (tenant_id) REFERENCES auth.tenant(tenant_id),
...
CREATE TABLE IF NOT EXISTS auth.usuario_rol (
    ...
    CONSTRAINT fk_auth_usuario_rol_usuario FOREIGN KEY (usuario_id) REFERENCES auth.usuario(usuario_id),
    CONSTRAINT fk_auth_usuario_rol_rol FOREIGN KEY (rol_id) REFERENCES auth.rol(rol_id)
);
...
CREATE TABLE IF NOT EXISTS auth.rol_permiso (
    ...
    CONSTRAINT fk_auth_rol_permiso_rol FOREIGN KEY (rol_id) REFERENCES auth.rol(rol_id),
    CONSTRAINT fk_auth_rol_permiso_permiso FOREIGN KEY (permiso_id) REFERENCES auth.permiso(permiso_id)
);
...
CREATE TABLE IF NOT EXISTS auth.refresh_token (
    ...
    CONSTRAINT fk_auth_refresh_usuario FOREIGN KEY (usuario_id) REFERENCES auth.usuario(usuario_id)
);

CREATE TABLE IF NOT EXISTS auth.login_auditoria (
    login_auditoria_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    usuario_id          UUID,
    ...
    motivo_fallo        TEXT,
    creado_en           TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT fk_auth_login_usuario FOREIGN KEY (usuario_id) REFERENCES auth.usuario(usuario_id)
);
