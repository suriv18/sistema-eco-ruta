...
import pe.edu.unmsm.ciudadsana.rutas.application.port.in.CrearRutaUseCase;
...
@Component
public class CrearRutaCommandHandler implements CrearRutaUseCase {

    private final RutasPersistencePort rutasPersistencePort;
    private final RutasEventPublisherPort eventPublisher;

    public CrearRutaCommandHandler(RutasPersistencePort rutasPersistencePort,
    ...
    @Override
    public Result<RutaResponseDto> crear(CrearRutaCommand cmd) {
        RutaId id = RutaId.of(UUID.randomUUID());
        TenantId tenantId = TenantId.of(cmd.tenantId());
        TipoRuta tipoRuta = TipoRuta.valueOf(cmd.tipoRuta());
