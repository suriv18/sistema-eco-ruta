...
class QuitarPermisoDeRolCommandHandlerTest {
    ...
    void quitar_rolYPermisoExisten_retornaSuccess() {
        ...
        Result<Void> result = handler.quitar(new QuitarPermisoDeRolCommand(ROL_ID, PERMISO_ID));

        assertThat(result.isSuccess()).isTrue();
        verify(rolPort).quitarPermiso(ROL_ID, PERMISO_ID);
    }

    @Test
    void quitar_rolNoExiste_retornaRolNoEncontrado() {
    ...
    @Test
    void quitar_permisoNoExiste_retornaPermisoNoEncontrado() {
        Rol rol = Rol.reconstitute(RolId.of(ROL_ID), "ADMIN", "Admin", "desc", true);
        when(rolPort.findById(any())).thenReturn(Optional.of(rol));
