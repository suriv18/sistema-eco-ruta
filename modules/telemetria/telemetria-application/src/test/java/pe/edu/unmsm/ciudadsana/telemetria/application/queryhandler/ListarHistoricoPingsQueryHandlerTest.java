...
class ListarHistoricoPingsQueryHandlerTest {

    @Mock
    PingGpsPersistencePort port;

    @InjectMocks
    ListarHistoricoPingsQueryHandler handler;

    // Municipalidad Metropolitana de Lima (tenant)
    private static final UUID TENANT_ID = UUID.fromString("11111111-1111-1111-1111-111111111111");
    // Camión compactador RIJ-0234 - ruta nocturna Barranco
    private static final UUID UNIDAD_ID = UUID.randomUUID();
    private static final UUID PING_ID = UUID.randomUUID();
    // Dispositivo Teltonika FMB920 instalado en unidad
    ...
    private PingGps pingMock() {
        ...
        // Coordenadas reales: Av. Grau con Av. República de Panamá, Barranco, Lima
        when(ping.getPosicion()).thenReturn(Coordenadas.of(-12.134667, -77.032500));
        // Velocidad típica de camión recolector en Barranco (zona con tráfico nocturno bajo)
        when(ping.getVelocidadKmh()).thenReturn(Optional.of(18.5));
        when(ping.getRumboGrados()).thenReturn(Optional.of(270.0)); // dirección oeste
        when(ping.getPrecisionM()).thenReturn(Optional.of(4.5));
    ...
    @Test
    void listar_conUnidadId_retornaPings() {
        PageResult<PingGps> page = PageResult.of(List.of(pingMock()), 0, 20, 1L);
        when(port.findHistorico(any(TenantId.class), any(), any(), any(), anyInt(), anyInt()))
                .thenReturn(page);

        Result<PageResult<PingGpsResponseDto>> result = handler.listar(
                new HistoricoPingsQuery(TENANT_ID, UNIDAD_ID, null, null, 0, 20));
    ...
    @Test
    void listar_sinFiltroUnidad_retornaTodosLosPings() {
        PageResult<PingGps> page = PageResult.of(List.of(pingMock()), 0, 20, 1L);
        when(port.findHistorico(any(TenantId.class), any(), any(), any(), anyInt(), anyInt()))
                .thenReturn(page);

        Result<PageResult<PingGpsResponseDto>> result = handler.listar(
                new HistoricoPingsQuery(TENANT_ID, null, null, null, 0, 20));
